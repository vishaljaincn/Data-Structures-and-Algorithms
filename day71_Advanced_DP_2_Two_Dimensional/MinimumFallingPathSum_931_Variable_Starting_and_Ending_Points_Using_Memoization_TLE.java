...
class MinimumFallingPathSum_931_Variable_Starting_and_Ending_Points_Using_Memoization_TLE {
    ...
    static int getMinFallingUtil(int i, int j, int n, int[][] matrix, int[][] dp) {
        ...
        // Calculate three possible paths: moving left, left diagonal, and right diagonal
        int left = matrix[i][j] + getMinFallingUtil(i - 1, j, n, matrix, dp);
        ...
        return dp[i][j] = Math.min(left, Math.min(leftDiagonal, rightDiagonal));
    }
