...
public class Gas_Station_134 {
    public int canCompleteCircuit(int[] gas, int[] cost) {
        int totalGas = 0; // Total gas available in the entire circuit
        int totalCost = 0; // Total cost of traveling the entire circuit
        int tank = 0; // Current gas tank level
        ...
        for (int i = 0; i < gas.length; i++) {
            ...
            // If tank becomes negative, reset start to the next station
            if (tank < 0) {
                tank = 0;
                start = i + 1;
            }
